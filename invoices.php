...
$query = $myPDO->prepare("SELECT `COLUMN_NAME` FROM `information_schema`.`COLUMNS` WHERE TABLE_NAME = 'Invoices' AND TABLE_SCHEMA = 'AZG'");
$query->execute();
$index=0;
...
$query = $myPDO->prepare("SELECT COUNT(*) CNT FROM `information_schema`.`COLUMNS` WHERE TABLE_NAME = 'Invoices' AND TABLE_SCHEMA = 'AZG'");
$query->execute();
$index=0;
while($row = $query->fetch(PDO::FETCH_ASSOC)){
...
$query = $myPDO->prepare("SELECT * FROM Invoices");
$query->execute();
$index=0;
while($row = $query->fetch(PDO::FETCH_ASSOC)){
...
$query = $myPDO->prepare("SELECT * FROM Invoice_Items");
$query->execute();
$list_of_AZ="";
$index=0;
while($row = $query->fetch(PDO::FETCH_ASSOC)){
  $list_of_AZ = $list_of_AZ.$row["AZ INVOICE"];
}
?>
<div id="content">
  ...
  <div class='metadata'>
    <label for="InvoiceDropdown">AZ Invoice&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
  ...
  <option value="" disabled selected>Select Invoice</option>
    ...
    $query->execute();
    while($row = $query->fetch(PDO::FETCH_ASSOC)){
      echo "<option value=$index>" . $row["AZ INVOICE"] . "</option>";
      $index++;
    }
?>
  </select>
...
<button type="button" onclick=redirectJS("new_invoice.php") class="newFormButton">New</button>
<button type="button" onclick="postInvoice()" class="newFormButton">Post Invoice</button>
</div>

<script>
var InvoiceOption='';
var ItemID='';
var Number_Of_Cartons='';
var Price_Per_LB='';
...
var InvoiceOptionNumber = Dropdown.options[Dropdown.selectedIndex].value;
InvoiceOption = results_array[InvoiceOptionNumber]['AZ INVOICE'];
...
$.post('backbones/update_tables/update_table_invoice.php', {invoice:InvoiceOption}, function(data) {
console.log(data);
var results = $.parseJSON(data);

var AZ_PO = results[0];
ItemID = results[1];
Number_Of_Cartons = results[2];
Price_Per_LB = results[3];
...
$('#data-table-body').append('<tr><td><center>'+AZ_PO[counter]+'</center></td><td><center>'+ItemID[counter]+'</center></td><td><center>'+Number_Of_Cartons[counter]+'</center></td><td><center>'+Price_Per_LB[counter]+'</center></td></tr>');
}
}
...
function postInvoice(){
  if(InvoiceOption==''){
    alert("Please choose an invoice first.");
  }
  else{
    $.post('backbones/inventory/post_invoice.php', {invoice:InvoiceOption}, function(data) {
      var results=$.parseJSON(data);
    });
