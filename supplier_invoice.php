<?php
$pageName="Supplier Invoices";
$roleNeeded=0;
include('backbones/includes/sidebar.php');

$query = $myPDO->prepare("SELECT `COLUMN_NAME` FROM `information_schema`.`COLUMNS` WHERE TABLE_NAME = 'Supplier_Invoice' AND TABLE_SCHEMA = 'AZG'");
$query->execute();
$index=0;
...
$query = $myPDO->prepare("SELECT COUNT(*) CNT FROM `information_schema`.`COLUMNS` WHERE TABLE_NAME = 'Supplier_Invoice' AND TABLE_SCHEMA = 'AZG'");
$query->execute();
$index=0;
while($row = $query->fetch(PDO::FETCH_ASSOC)){
...
$query = $myPDO->prepare("SELECT * FROM Supplier_Invoice");
$query->execute();
$index=0;
while($row = $query->fetch(PDO::FETCH_ASSOC)){
...
$query = $myPDO->prepare("SELECT * FROM Supplier_Invoice_Items");
$query->execute();
$list_of_AZ="";
$index=0;
while($row = $query->fetch(PDO::FETCH_ASSOC)){
  $list_of_AZ = $list_of_AZ.$row["Supplier Invoice ID"];
}
?>
<div id="content">
  ...
  <div class='metadata'>
    <label for="InvoiceDropdown">Supplier Invoice&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
  ...
  <option value="" disabled selected>Select Invoice</option>
    ...
    $query->execute();
    while($row = $query->fetch(PDO::FETCH_ASSOC)){
      echo "<option value=$index>" . $row["Supplier Invoice ID"] . "</option>";
      $index++;
    }
...
<button type="button" onclick=redirectJS("new_supplier_invoice.php") class="newFormButton">New</button>
</div>
...
var InvoiceOption = results_array[InvoiceOptionNumber]['Supplier Invoice ID'];
...
console.log("No invoice named " + InvoiceOption + " in table Supplier_Invoice");
}

$.post('backbones/update_tables/update_table_supplier_invoice.php', {invoice:InvoiceOption}, function(data) {
console.log(data);
