...
$query = $myPDO->prepare("SELECT DISTINCT Item_ID FROM `Items`");
$query->execute();
$index=0;
while($row = $query->fetch(PDO::FETCH_ASSOC)){
...
<form action="insertNewCustomerPO.php" method="post" onSubmit="return validateForm()">
  <div class='metadata'>
  ...
  <div class='metadata'>
    <label for="Billing_Address_ID">Billing Address ID</label>
    <input type='text' id="Billing_Address_ID" name='Billing_Address_ID'>
  </div>
  <div class='metadata'>
...
<datalist id="test">
        ...
        foreach($Items_Array as $option){?>
          <option value="<?php echo $option['Item_ID'] ?>"><?php echo $option['Item_ID'] ?></option>
      <?php
      } ?>
      </datalist>
    <!-- </select> -->
